#include "utils.h"
	...
	void utils::generateInitX(const string& xsFilename, int rows, double initNum) {
	...
	void utils::generateMatrix(const string& matrixFilename, int rows) {
	...
	tuple<double*, double*, double*, int> utils::load(const string& matrixFilename, const string& xsFilename) {
		int rows, columns, m;
	...
	void utils::printVector(char* title, double xLocal[], int n)
	{
		printf("%s\n", title);
		for (int i = 0; i < n; i++)
	...
	void utils::printResultInFile(const string& outFile, double* matrix, int n) {
