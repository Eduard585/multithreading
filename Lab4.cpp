...
void createMatrix(string filename, int nodesSize, int minBorder, int maxBorder);
void printAnswer(int root, const string& outFile, const vector<int>& distances);
void writeLogTime(string& str);
vector<int> dijkstra(graph graph, long startNode, long chunkSize);

int main(int argc, char* argv[])
{
	if (argc == 5) {
		createMatrix(argv[1], atoi(argv[2]), atoi(argv[3]), atoi(argv[4]));
	}
	else if (argc < 6) {
		...
		distances = dijkstra(graph, startNode, chunkSize);

		timeMultiply = GetTickCount() - startTime;
		res = snprintf(buf, sizeof(buf), "%d threads: %Id size, spent %Id milliseconds\n", threadNum, distances.size(), timeMultiply);
		string str = "error!";
